...
import { Lots22Service, SecuritiesOwner, SecuritiesOwnerDeadAttachment } from './lots22.service';
import { Lots22BorrowerLookupComponent } from './lots22-borrower-lookup.component';

@Component({
  templateUrl: './lots22-detail.component.html'
})
export class Lots22DetailComponent implements OnInit {
  ...
  Lots22BorrowerLookupContent = Lots22BorrowerLookupComponent;
  ContractAttachment = [];
  ...
  ngOnInit() {
    this.route.data.subscribe((data) => {
      this.statusReq = data.Lots22.master.ReqStatus;
      if (data.Lots22.securitiesOwnerDeadDetail.SecuritiesOwnerDead) {
        this.detailOwnerList = data.Lots22.securitiesOwnerDeadDetail.SecuritiesOwnerDead;
      }
      this.ContractAttachment = data.Lots22.securitiesOwnerDeadDetail.ContractAttachment ? data.Lots22.securitiesOwnerDeadDetail.ContractAttachment : [];
    });
  ...
  rebuildForm() {
    ...
    this.DetailOwnerForm.controls.DeadRemark.enable({ emitEvent: false })
    ...
    if (this.detailOwnerList.SecuritiesOwnerDeadId) {
      ...
      if (this.DetailOwnerForm.controls.ReqStatus.value == '1' || this.DetailOwnerForm.controls.ReqStatus.value == '2') {
        this.DetailOwnerForm.controls.OwnerDeadDate.enable({ emitEvent: false })
        this.DetailOwnerForm.controls.DeadRemark.enable({ emitEvent: false })
      } else {
  ...
  fileNameReturn2(filename, index) {
    let form = this.contractAttachments2.controls[index] as FormGroup;
    form.controls.FileName.setValue(filename);
  }
    ...
    if (loanDetail != undefined) {
      this.is.getBorrowerDetail(loanDetail)
        ...
        .subscribe(
          (res: any) => {
            if (!this.detailOwnerList.SecuritiesOwnerDeadId) {
              this.DetailOwnerForm.controls.MainContractNo.reset();
            }
            ...
            this.contractNoDetail(res.CustomerCode, this.detailOwnerList.SecuritiesOwnerDeadId)
            if (this.ContractAttachment.length <= 0) {
              this.DetailOwnerForm.setControl('ContractAttachment', this.fb.array([]));
            }
          });
    }
  }

  contractNoDetail(CustomerCode, SecuritiesOwnerDeadId) {
    this.is.getContractNoDetail(CustomerCode, SecuritiesOwnerDeadId)
      .pipe(finalize(() => {
      }))
      .subscribe(
        (res: any) => {
  ...
  back() {
    this.router.navigate(['lo/lots22'], { skipLocationChange: true });
  }

  getChangeMainContractNo(val) {
    if (val) {
      this.DetailOwnerForm.controls.MainContractHeadId.setValue(val.ContractHeadId);

      this.is.getContractAttachment(this.DetailOwnerForm.controls.MainContractHeadId.value).pipe()
        .subscribe(
          (res: any) => {
            this.DetailOwnerForm.setControl('ContractAttachment', this.fb.array(res.ContractAttachment.map((detail) => this.contractAttachmentsForm2(detail))))
            this.checkContractAttachment2();
          });
    } else {
      this.DetailOwnerForm.setControl('ContractAttachment', this.fb.array([]));
