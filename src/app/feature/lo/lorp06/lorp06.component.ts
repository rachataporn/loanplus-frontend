...
import { Lorp06Service, ReportParam } from '@app/feature/lo/lorp06/lorp06.service';
import { Page, ModalService, SelectFilterService } from '@app/shared';
import { AlertService, LangService, SaveDataService } from '@app/core';
import { moment } from 'ngx-bootstrap/chronos/test/chain';
@Component({
  templateUrl: './lorp06.component.html'
})

export class Lorp06Component implements OnInit {
  reportParam = {} as ReportParam
  loanApprovedAeportForm: FormGroup; 
  fromBranchList = [];
  toBranchList = [];
  ...
  constructor(
    ...
    private lorp06Service: Lorp06Service,
  ...
  ngOnInit() {
    const saveData = this.saveData.retrive('LORP06');
    ...
    this.route.data.subscribe((data) => {
      ...
      this.fromBranchList = data.lorp06.FromBranch;  
      this.toBranchList = data.lorp06.ToBranch;
      this.fromContractTypeList = data.lorp06.FromContractType;
      this.toContractTypeList = data.lorp06.ToContractType;
  ...
  filterToLoanType(filter?: boolean) {
   ...
   ngOnDestroy() {
    this.saveData.save(this.searchForm.value, 'LORP06');
  }
  print(){
    ...
    if (branch > 0) this.as.warning('', 'Message.LO00006', ['Label.LORP06.Branch', 'Label.LORP06.ToBranch']);
    if (loanType > 0) this.as.warning('', 'Message.LO00006', ['Label.LORP06.FromContractType', 'Label.LORP06.ToContractType']);
    if (branch == 1 ||  loanType == 1  > 0) return;
    Object.assign(this.reportParam, this.searchForm.value);
    this.reportParam.ReportName = 'LORP06';
    ...
    this.lorp06Service.generateReport(this.reportParam).pipe(
      finalize(() => {
      })
    )
      .subscribe((res) => {
