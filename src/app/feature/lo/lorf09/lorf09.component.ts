...
import { Lorf09Service, ReportParam } from '@app/feature/lo/lorf09/lorf09.service';
import { Page, ModalService, SelectFilterService } from '@app/shared';
import { AlertService, LangService, SaveDataService } from '@app/core';
import { from } from 'rxjs';

@Component({
  templateUrl: './lorf09.component.html'
})

export class Lorf09Component implements OnInit {
  ...
  ngOnInit() {
    const saveData = this.saveData.retrive('LORF09');
    ...
    this.route.data.subscribe((data) => {
      this.fromBranchList = data.garuntee.Branch;
      this.toBranchList = data.garuntee.Branch;
      this.fromCustomerCodeList = data.garuntee.BorrowerCode;
      this.toCustomerCodeList = data.garuntee.BorrowerCode;
      this.contractNoList = data.garuntee.ContractNo;
      this.fromContractNoList = data.garuntee.ContractNo;
      this.toContractNoList = data.garuntee.ContractNo;
      this.bindDropDownList();
    });
    ...
    if (routeFromContractNo && routeToContractNo) {
      ...
      this.saveData.save(this.searchForm.value, "LORF09");
  ...
  print() {
    ...
    if (branch > 0) this.as.warning('', 'Message.LO00006', ['Label.LORF09.FromBranch', 'Label.LORF09.ToBranch']);
    if (contractNo > 0) this.as.warning('', 'Message.LO00006', ['Label.LORF09.FromContractNo', 'Label.LORF09.ToContractNo']);
    if (customerCode > 0) this.as.warning('', 'Message.LO00006', ['Label.LORP09.FromCustomerCode', 'Label.LORP09.ToCustomerCode']);
