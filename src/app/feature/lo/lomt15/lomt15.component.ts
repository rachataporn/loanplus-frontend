...
import { Lomt15Service } from '@app/feature/lo/lomt15/lomt15.service';
import { Page, TableService, ModalService } from '@app/shared';
import { AlertService, LangService, SaveDataService } from '@app/core';

@Component({
  templateUrl: './lomt15.component.html'
})

export class Lomt15Component implements OnInit {
  transport = [];
  page = new Page();
  statusPage: boolean;
  ...
  onSearch() {
    this.service.getTransport(this.statusPage ? (this.searchForm.value) : this.beforeSearch, this.page).pipe(finalize(() => {
      if (this.statusPage) {
      ...
      .subscribe(
        (res: any) => {
          this.transport = res.Rows;
          this.page.totalElements = res.Rows.length ? res.Total : 0;
        });
  }

  add() {
    this.router.navigate(['/lo/lomt15/detail'], { skipLocationChange: true });
  }

  edit(id) {
    this.router.navigate(['/lo/lomt15/detail', { id: id }], { skipLocationChange: true });
  }

  remove(TransportId: number, RowVersion) {
    this.modal.confirm('Message.STD00003').subscribe(
      (res) => {
        if (res) {
          this.service.deleteTransport(TransportId, RowVersion).subscribe(
            (response) => {
              if (response) {
