...
import { Dbmt17Service } from '@app/feature/db/dbmt17/dbmt17.service';
import { Page, ModalService } from '@app/shared';
import { AlertService, LangService, SaveDataService } from '@app/core';

@Component({
  templateUrl: './dbmt17.component.html'
})

export class Dbmt17Component implements OnInit {
  countryList = [];
  count = 0;
  page = new Page();
  ...
  constructor(
    ...
    private dbmt17Service: Dbmt17Service,
  ...
  onSearch() {
    ...
    this.dbmt17Service.getData(this.statusPage ? (this.searchForm.value) : this.beforeSearch, this.page)
      .pipe(finalize(() => {
        if (this.statusPage) {
  ...
  add() {
    this.router.navigate(['/db/dbmt17/detail', { flagSearch: true }], { skipLocationChange: true });
  }

  edit(CountryCode) {
    this.router.navigate(['/db/dbmt17/detail', { CountryCode: CountryCode }], { skipLocationChange: true });
  }
  ...
  remove(CountryCode, RowVersion) {
    this.modal.confirm('Message.STD000030').subscribe(
      (res) => {
        if (res) {
          this.dbmt17Service.deleteCountry(CountryCode, RowVersion).subscribe(
            (response) => {
