...
import { Dbmt09Service } from '@app/feature/db/dbmt09/dbmt09.service';
import { Page, ModalService } from '@app/shared';
import { AlertService, LangService, SaveDataService } from '@app/core';

@Component({
  templateUrl: './dbmt09.component.html'
})

export class Dbmt09Component implements OnInit {
  countryList = [];
  count = 0;
  page = new Page();
  ...
  constructor(
    ...
    private dbmt09Service: Dbmt09Service,
  ...
  onSearch() {
    ...
    this.dbmt09Service.getData(this.statusPage ? (this.searchForm.value) : this.beforeSearch, this.page)
      .pipe(finalize(() => {
        if (this.statusPage) {
  ...
  add() {
    this.router.navigate(['/db/dbmt09/detail', { flagSearch: true }], { skipLocationChange: true });
  }

  edit(CountryCode) {
    this.router.navigate(['/db/dbmt09/detail', { CountryCode: CountryCode }], { skipLocationChange: true });
  }
  ...
  remove(CountryCode, RowVersion) {
    this.modal.confirm('Message.STD000030').subscribe(
      (res) => {
        if (res) {
          this.dbmt09Service.deleteCountry(CountryCode, RowVersion).subscribe(
            (response) => {
